...
export default function SelectTokenModal() {
  ...
  const sortedAndLikedTokenList = useMemo(() => {
    ...
    if (
      ...
      !customOrder.includes(filteredTokenList[0].tokenSymbol as string)
    ) {
    ...
    if (likeList.length > 0) {
      ...
      const unlikedTokens = orderedTokens
        .filter(
          (token) =>
            !likeList.some(
              (like) =>
  ...
  useEffect(() => {
    ...
    if (network.connectedChainId) {
      return setSearchToken({
        ...
        chainId: network.connectedChainId,
      });
    }
  }, [searchValue]);
        ...
        <HStack justifyContent="flex-end">
          ...
          {tokenData.isLiked !== "none" && (
            <Box onClick={handleStarClick}>
              <Icon
                as={StarIcon}
                color={tokenData.isLiked === "true" ? "#007AFF" : "#A0A3AD"}
              />
            </Box>
          )}
        </HStack>
        ...
        <HStack justifyContent="space-between" px={2} pt={3}>
          ...
          <HStack spacing="2">
            <Flex
              ...
              bg={"#17181D"}
              border={
                !isActiveChain(network?.connectedChainId)
                  ? "1px solid #007AFF"
                  : ""
              }
              cursor={
                isActiveChain(network?.connectedChainId) ? "pointer" : "default"
              }
              onClick={
                isActiveChain(network?.connectedChainId)
                  ? changeNetwork
          ...
          {filteredTokenList.length === 1 && filteredTokenList[0].isNew ? (
            ...
            <VStack spacing={2}>
              {sortedAndLikedTokenList?.map((tokenData, index) => (
                <TokenListItem key={index} tokenData={tokenData} />
              ))}
            </VStack>
          )}
        </ModalBody>
